...
void readTrainingTest(FILE *file, char **types, int attrSize)
{
	...
	while(strcmp(line, "-1") != 0)
	{
		...
		/* Last element read must be a class; store it as char* */
		else if(count % (attrSize) == 0)
		{
			if(strstr(types[i], "["))
			{
				num = getSize(types[i]);
...
int getDataSize(FILE *file, int attrSize, char **types)
{
	...
	dataSize = ftell(file);
	dataSize = (int)ceil((double)dataSize/getSchemaSize(types, attrSize));
	rewind(file);
	return dataSize;
}
		...
		for(j = 0; j < (attrSize+2); j++)
		{
			...
			else
			{
				...
				if(j % (attrSize) == 0)
				{
					...
					printf("vem aqui showtime\n");
					printf("%s = %s\n", names[j-1], str);
					free(str);
				}
				else if(strcmp(types[j-1], "int") == 0)
				{
					integer = (int*)malloc(sizeof(int));
					fread(integer, sizeof(int), 1, file);
					printf("%s = %d\n", names[j-1], (*integer));
	...
	for(i = 0; i < (attrSize+2); i++)
	{
		...
		else
		{
			...
			if(strcmp(types[i-1], "int") == 0)
			{
				...
				fread((*binaryInt), sizeof(int), 1, file);
...
char **retrieveClasses(FILE *file, int kValue, dist *distances, char **types, int attrSize)
{
	...
	{
		rewind(file);
		fseek(file, (distances[i].index*getSchemaSize(types, attrSize)), SEEK_SET);
...
void dump_nn(char *table, FILE *file, int attrSize, char **names, char **types, int **nearestNeighbors)
{
	...
	{
		rewind(file);
		fseek(file, ((*nearestNeighbors)[i]*getSchemaSize(types, attrSize)), SEEK_SET);
		readBinaryEntry(file, attrSize, types, &binaryId, &binaryInt, &binaryIntSize, &binaryDouble, &binaryDoubleSize, &class, &binaryDist);
...
int main(void)
{
	...
	/* schemaName - ".schema" extension file name; schema - only ".schema" name, no extension;
		...
		(dump_data), (dump_nn) or (knn) */
	char *schemaName, *schema, **names, **types, *tableName, *table, *command;
	...
	if(fSchema)
	{
		...
		readNameAndType(fSchema, names, types, attrSize);
		// printf("names:\n");
		...
		while(strcmp(command, "exit") != 0)
		{
			...
			{
				dumpSchema(tableName, attrSize, names, types);
			}
			/* dump_data command */
