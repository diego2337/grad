...
/*
	...
	params:
	   ...
	   - size: size of amp
*/
void writeAudio(unsigned char *amp, int size)
{
	// FILE *output = fopen("saida.raw", "wb");
...
unsigned char *kmeans(unsigned char *amp, int size, int k, double *groups, double t)
{
	...
	double min, max, sum, changeSum, *changes, value;
	...
	for(i = 0; i < size; i++)
	{
		...
		for(j = 0; j < k; j++)
		{
			...
			if((amp[i]-groups[j]) < 0.0000000000)
			{
				value = sqrt(fabs((amp[i]-groups[j])));
				if(value < max)
				{
					max = value;
					index = j;
	...
	for(i = 0; i < k; i++)
	{
		...
		if(sum_count != 0)
		{
			changes[i] = fabs(groups[i]-(sum/sum_count));
			changeSum = changeSum + changes[i];
			groups[i] = (sum/sum_count);
		}
		else
		{
			changes[i] = groups[i];
	...
	// printVector2(groups, k);
	changeSum = (changeSum/k);
